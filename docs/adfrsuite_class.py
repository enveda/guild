...
from MolKit.bondSelector import (
    AmideBondSelector,
    GuanidiniumBondSelector,
    RotatableBondSelector,
)
...
from MolKit.pdbWriter import PdbqsWriter, PdbqtWriter, PdbqWriter
...
class AutoDockMoleculePreparation:
    ...
    def repairMol(self, mol, repair_list):
        ...
        if "bonds" in repair_list:
            ...
            if non_bonded_atoms:
                if self.debug:
                    print("!!adding bonds to ", non_bonded_atoms.name, "!!")
    ...
    def addCharges(self, mol, charges_to_add):
        ...
        if debug:
            print("self.chargeType=", self.chargeType)
        if not self.chargeType:
            len_zero_charges = len(
                [x for x in mol.chains.residues.atoms if x.chargeSet and x.charge == 0]
            )
            if len_zero_charges == len(mol.allAtoms):
                print(
                    "WARNING: all atoms in '%s' had zero charges! Adding gasteiger charges..."
                    % mol.name
                )
                ...
                if len_charged != len(mol.chains.residues.atoms):
                    ...
                    print(
                        f"WARNING: some atoms in '{mol.name}' had no charge! Adding gasteiger charges to all..."
                    )
                    self.chargeCalculator.addCharges(mol.allAtoms)
                if debug:
                    print("no change to charges!")
            self.chargeType = mol.allAtoms[0].chargeSet
        else:
            self.chargeCalculator.addCharges(mol.allAtoms)
            if debug:
                print("added " + charges_to_add + "charges to ", mol.name)
    ...
    def cleanUpAtomTypes(self, mol, cleanup_list):
        ...
        if merge_nonpolar_hydrogens:
            ...
            if debug:
                print("merged ", lenNPHS, " nonpolar hydrogens")
        ...
        if delete_alternateB:
            ...
            if self.debug:
                print("found alt_ats=", len(alt_ats), " to delete")
            if not len(alt_ats):
                if self.debug:
                    print("no @ats found")
                return
            ...
            for t in alt_types[1:]:
                ...
                if self.debug:
                    print(alt_s, "now removing ", len(t_alt_ats), " atoms:")
                ct = 0
                for a in t_alt_ats:
                    res = a.parent
                    for b in a.bonds:
                    ...
                    ct = ct + 1
                if self.debug:
                    print("removed ", ct, " atoms:")
            count = 0
            # rename any remaining @
            for a in altA_ats:
                ind = a.name.index("@")
                if self.debug:
                    print("changing name of ", a.name, end=" ")
                a.name = a.name[:ind]
                if self.debug:
                    print(" to ", a.name)
                count = count + 1
            if self.debug:
                print("end of processing altA_ats: count=", count)
            mol.allAtoms = mol.chains.residues.atoms
            if self.debug:
                print("resetting numbers to 1-", len(mol.allAtoms) + 1)
...
class ReceptorPreparation(AutoDockMoleculePreparation):
    ...
    def __init__(
        ...
        if len(preserved):
            for atom, chargeList in list(preserved.items()):
                ...
                if debug:
                    print("preserved charge on ", atom.name, " charge=", atom.charge)
    ...
    def write(self, outputfilename=None):
        ...
        if not outputfilename and not self.outputfilename:
            ...
            if self.debug:
                print("using std outputfilename ", outputfilename)
        ...
        if self.debug:
            print("wrote ", mol.name, " to ", outputfilename)
        ...
        for atm in atms:
            ...
            for b in atm.bonds:
                ...
                if a2 == atm:
                    a2 = b.atom2
                if a2 not in atms:
                    continue  # don't write intermolecular bonds
...
class LigandPreparation(AutoDockMoleculePreparation):
    ...
    def restore_autodock_element_types_after_writing(self, problem_ats):
        for at in problem_ats:
            # there is a substitute 'name' for this atom
            if self.debug:
                print("changed back", at.name, "'s element ")
    ...
    def set_all_torsions(self, flag):
        mol = self.molecule
        if self.debug:
            print("LPO:set_all_torsions with flag=", flag)
        if self.debug:
            print("activeBonds=", len([x for x in mol.allAtoms.bonds[0] if x.activeTors]))
        self.RBM.set_all_torsions(flag)
        if self.debug:
            print(
                "2: activeBonds=",
                len([x for x in mol.allAtoms.bonds[0] if x.activeTors]),
            )
    ...
    def set_carbon_names(self, atoms, type):
        ...
        self.aromCs = AtomSet(
            [x for x in self.molecule.allAtoms if (x.autodock_element == "A" and x.element == "C")]
        )
...
class AD4LigandPreparation(LigandPreparation):
    ...
    def __init__(
        ...
        version=4,
        typeAtoms=True,
        limit_torsion_number=False,
        limit_torsions_type="fewest",
        limit_torsions=False,
        delete_single_nonstd_residues=False,
        ...
        molecule.TORSDOF = molecule.possible_tors - len(
            molecule.guanidiniumbnds + molecule.amidebnds
        )
...
class LigandWriter:
        ...
        for at in rootlist:
            ...
            for bond in at.bonds:
                ...
                if at2 not in rootlist:
                    rootlist.append(at2)
                    if self.debug:
                        print("added ", at2.name, " to rootlist")
                    at2.rnum0 = rootnum
                    rootnum = rootnum + 1
    ...
    def process(self, fromAtom, nextAtom):
        ...
        if self.debug:
            print(fromAtom.name, ":", nextAtom.name, ": queue=", queue)
        if len(queue):
            for fromAtom, nextAtom in queue:
                if self.debug:
                    print(" processing queue entry: ", fromAtom.name, "-", nextAtom.name)
    ...
    def writeLevel(self, atom, outfptr):
        ...
        for b in atom.bonds:
            ...
            if b.activeTors:
                ...
                if self.debug:
                    print(atom.name, "wL: queue=", queue)
                continue
            a2 = b.atom1
            if a2 == atom:
                a2 = b.atom2
        ...
        for a2 in nextAts:
            if self.debug:
                print("for nextAts loop with a2=", a2.name, "calling wL")
            nq = self.writeLevel(a2, outfptr)
            if len(nq):
                if self.debug:
                    print("extending queue with", nq)
                queue.extend(nq)
        if self.debug:
    ...
    def writeBreadthFirst(self, outfptr, fromAtom, startAtom):
        ...
        if startAtom.used == 0:
            ...
            if self.debug:
                print("wBF: wrote ", startAtom.name)
            ...
            for bond in startAtom.bonds:
                ...
                if at2 == fromAtom:
                    ...
                    if bond.activeTors:
                        ...
                        if self.debug:
                            print(
                                "\n\nwriting and calling wL with nA=",
                                at2.name,
                                "-",
                                at2.number,
                            )
...
class LigandRandomizer:
    ...
    def QuatToAxisAngle(self, q):
        ...
        inv_sin_half_angle = 1.0 / math.sin(angle / 2.0)
        quat = []
    ...
    def __init__(
        self,
        molecule,
        outputfilename=None,  # TRANSLATION
        ...
        if verbose:
            print("IN LIGAND RANDOMIZER: m.mol_cen=", m.mol_cen)
        ...
        if torsOnly:
            ...
            for new_try in range(ntries):
                ...
                random_sign = +1
                if r1 < 0.5:
                    random_sign = -1  # from constants.h
                PI = math.pi
                TWOPI = 2.0 * PI  # TWOPI = 6.28318530717958647692  from autocomm.h
                ...
                t2 = TWOPI * random.uniform(0.0, 1.0)
                # now.Q.z
                ...
                # check for identity
                if abs(w) > 1.001:
                    raise Exception("too big W")  # ?@@?
                if w > 1.0:
                    w = 1.0
                if w < -1.0:
                    w = -1.0  # if abs(w)==1 return sidentityAxisAngle==[1.,0.,0.,0.]
                ...
                if mag3 > 0.0000001:
                    ...
                    axisangle.append(angle * 360.0 / (2 * math.pi))
                ...
                if verbose:
                    print(
                        "originally %d bonds form; after transformation %d bonds form"
                        % (orig, newLen)
                    )
                new_d = {}
                for a in m.allAtoms:
                    new_d[a] = set(a.bonds.getAtoms())
                ok = True
                for a in m.allAtoms:
                    if orig_d[a] != new_d[a]:
                        ok = False
                        if verbose:
                            print(
                                "bonds differ for %s: %d v %d"
                                % (a.full_name(), len(orig_d[a]), len(new_d[a]))
                            )
                if verbose:
                    print("end of try ", new_try + 1)
                if ok:
                    if verbose:
            ...
            if ok:
                ...
                if verbose:
                    print(
                        "no new contacts formed so writing file after ",
                        new_try + 1,
                        end=" ",
                    )
                    if new_try == 0:
                ...
                if verbose:
                    print("now write ", outputfilename, " @@")
                m.parser.write_with_new_coords(newCrds, outputfilename)
                if verbose:
                    print("wrote new conformation in ", outputfilename)
                self.ok = 1
            else:
                self.ok = 0
                print("FAILED! no new conformation found in %d attempts!!!" % ntries)
            if verbose:
                print("END OF LIGAND RANDOMIZATION!!!")
...
class RotatableBondManager:
    ...
    def __init__(
        ...
        if debug:
            print("allow amide=", allow_amide_torsions)
        ...
        if len(bonds_to_inactivate):
            ...
            if debug:
                print("bnds_list=", bnds_list)
            for i in range(len(bnds_list) / 2):
                ind1 = bnds_list[i * 2]
                ind2 = bnds_list[i * 2 + 1]
                if debug:
                    print("inactivating ", ind1, " to ", ind2)
    ...
    def __classifyBonds(self, atoms, allow_guanidinium_torsions):
        ...
        if len(dict["rotatable"]):
            if self.debug:
                print("len(dict['rotatable'])=", len(dict["rotatable"]))
        ...
        for b in dict["guanidinium"]:
            ...
            if not b.incycle and not b.leaf and b.bondOrder == 1:
                if self.debug:
                    print("set ", b, ".possible/active to 1/0")
                b.possibleTors = 1
        ...
        if self.debug:
            print("set ", mol.name, ".torscount=", mol.torscount)
    ...
    def set_amide_torsions(self, flag):
        ...
        """
        if self.debug:
            print("in set_amide_torsions with flag", flag)
    ...
    def autoroot(self, verbose=0):
        ...
        for item in atoms_to_check:
            ...
            if hasattr(item, "leaf"):
                ...
                continue
            if verbose:
                print("processing atom ", item.number)
            item.maxbranch = 0
            for b in item.bonds:
                nxtAtom = b.atom1
                if nxtAtom == item:
        ...
        # if ties for possible root, use first entry in bestRoot list...
            ...
            if verbose:
                print("no entries in bestList using first atom ")
    ...
    def limit_torsions(self, numTors, type="fewest"):
        ...
        if not hasattr(mol, "ROOT"):
            ...
            return "ERROR"
        if not hasattr(mol, "torTree") or not hasattr(mol.allAtoms[0], "tt_ind"):
...
class AD4FlexibleDockingPreparation:
    ...
    def __init__(
        ...
        if mode == "automatic":
            ...
            if self.debug:
                print("wrote flexible file:", flex_filename)
            self.write_rigid(self.molecule, rigid_filename)
            if self.debug:
                print("wrote rigid file:", rigid_filename)
    ...
    def write(self, outputfilenames=None):
        if outputfilenames is not None and len(outputfilenames) == 2:
            ...
            if self.debug:
                print("using default filenames: ", end=" ")
            flex_filename = self.ligand.name + "_" + self.molecule.name + "_flex.pdbqt"
            rigid_filename = self.ligand.name + "_" + self.molecule.name + "_rigid.pdbqt"
            if self.debug:
                print(flex_filename, " and ", rigid_filename)
    ...
    def write_flex(self, flex_residues, ligandfile, outputfilename):
        ...
        for line in ligfileptr:
            ...
            if string.find(line, "HETA") > -1:
                self.outatom_counter += 1
            if string.find(line, "CONECT") > -1:
                continue
            if string.find(line, "MASTER") > -1:
                continue
        ...
        outfileptr.close()
        if self.debug:
            print("wrote flex file", outputfilename)
    ...
    def setAutoFlexFields(self, res):
        if self.debug:
            print("in setAutoFlexFields with ", res.full_name())
        ...
        if self.debug:
            print("len(rotatables)=", len(rotatables))
        for b in rotatables:
            b.possibleTors = 1
            b.activeTors = 1
...
class AD4FlexibleReceptorPreparation:
    ...
    def __init__(
        ...
        if self.debug:
            print("about to process_residues: ", residues)
        flex_residues = self.flex_residues = self.process_residues(residues)
        # MODE SWITCH 5: write outputfiles     ||IN USE||
        if mode == "automatic":
            # write outputfile now without waiting ...
            ...
            if self.debug:
                print("wrote flexible file:", flexres_filename)
    ...
    def write(self, outputfilenames=None):
        if outputfilenames is not None and len(outputfilenames) == 2:
            ...
            rigid_filename = self.receptor.name + "_rigid.pdbqt"
            if self.debug:
                print(flexres_filename, " and ", rigid_filename)
    ...
    def write_flex(self, flex_residues, flexres_filename):
        ...
        if self.debug:
            print(
                "in write_flex: len(flex_residues)=",
                len(flex_residues),
                "ffn=",
                flexres_filename,
            )
        ...
        if self.debug:
            print("wrote flex file", flexres_filename)
    ...
    def writeResidue(self, res, outfileptr):
        ...
        # start output
        if self.debug:
            print("in writeResidue with ", res.full_name())
        outfileptr.write("BEGIN_RES %s %s%4s\n" % (res.type, res.parent.id, res.number))
        # first write out remarks about torsions
        outfileptr.write("REMARK  " + "%d" % (res.torscount) + " active torsions:\n")
        outfileptr.write("REMARK  status: ('A' for Active; 'I' for Inactive)\n")
    ...
    def writeLevel(self, atom, outfptr):
        ...
        for b in atom.bonds:
            if b.atom1 in self.writtenAtoms and b.atom2 in self.writtenAtoms:
                if self.debug:
                    print("skipping b=", b.atom1.name, "-", b.atom2.name)
                continue
            if self.debug:
                print(
                    "processing b=",
            ...
            if a2.used:
                if self.debug:
                    print("!!a2 is already used!!", a2.name)
                continue
            if a2 not in self.writtenAtoms:
                a2.number = a2.newindex = self.outatom_counter
                if self.debug:
                    print(
                        "writeLevel: wrote bonded atom named=",
                        a2.name,
                        "a2.used=",
                        a2.used,
                    )
    ...
    def process_residues(self, residues):
        ...
        for res in residues:
            ...
            if res.type == "HOH":
                continue
            if self.debug:
                print("calling setAutoFlexFields with ", res.full_name())
            ntors = self.setAutoFlexFields(res)
            if self.debug:
                print("ntors=", ntors)
            # only keep residues with at least 1 torsion
