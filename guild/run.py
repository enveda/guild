...
from guild.transformers.pdb import LigandIdentifier, get_pocket_contacts_from_ligand
...
class Guild:
    ...
    def __init__(
        ...
        # Get first chain of the protein if no target chain is specified
        if protein_chain is None:
            self.protein_chain = get_protein_chain(self.original_protein_data, self.protein_idx)
        else:
        ...
        try:
            self._prepare_protein(target_chain=self.protein_chain)
            logger.info(f"Protein data prepared for chain {self.protein_chain}.")
        except Exception as e:
    ...
    def _prepare_ligand(self):
        ...
        if os.path.exists(self.vina_box):
    ...
    def _prepare_protein(self, target_chain=None):
        ...
        # Get the original sequence of the protein
        original_sequence_dictionary = get_original_sequence_dictionary(self.cleaned_protein)
        self.original_sequence = process_into_fasta_string(
            original_sequence_dictionary[self.protein_chain]
        )

        # If P2Rank binding site prediction is enabled, run it after protein is prepared
        if self.predict_binding_pocket:
            try:
    ...
    def run_boltz(self):
        ...
        print(f"MSA file for Boltz: {msa_file}")

        if self.original_ligand is not None and self.original_ligand_chain is not None:
        ...
        generate_boltz_yaml(
            protein_sequence=self.original_sequence,
            protein_chain=self.protein_chain,
